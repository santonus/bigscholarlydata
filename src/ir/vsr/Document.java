...
public abstract class Document {
	...
	//protected static final String stopWordsFile = "C:\\Users\\subhajit_d\\workspace\\Info_Retrieval\\src\\u\\mooney\\ir-code\\ir\\utilities\\stopwords.txt";
	protected static final String stopWordsFile = "config/stopwords.txt";
	...
	/**
	 * Creates a new Document making sure that the stopwords are loaded,
	 * indexed, and ready for use. Subclasses that create concrete instances
	 * MUST call prepareNextToken before finishing to ensure that the first
	 * token is precomputed and available.
	 */
	...
	/**
	 * Return the next possible token in the document. Each subclass must
	 * implement this method to produce candidate tokens for subsequent
	 * stop-word filtering.
	 */
	protected abstract String getNextCandidateToken();

	/**
	 * Returns the total number of tokens in the document or -1 if there are
	 * still more tokens to be read and the total count is not yet available.
	 */
	...
	protected void loadStopWords() {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				stopWords.add(processword(line));
			}
			in.close();
		} catch (IOException e) {
			System.out.println("\nCould not load stopwords file: "
					+ stopWordsFile);
			System.exit(1);
		}
	}
	/** Added by Santonu. This is the default way of preprocessing a stopword ***/
