...
public class CVLearningCurve
{
     ...
     * @param c BatchClassifier on which to perform K-fold CV
  ...
  public void trainAndTestFold(Vector<Example> train, Vector<Example> test, int fold, 
	...
	for (int j=0; j<numFolds; j++) {
		...
		if ((train.size() +  foldSize) <= numTrain) {
			...
			train.addAll((Vector<Example>)foldBins[i][j]);
		    }
		}
		...
		// train data
			...
			for (int k=0; k<len; k++) {
			    train.add((Example)foldBins[i][j].get(k));
			}
		    }
		    break;
		}
	...
	for (int i=0; i<numClasses; i++)
	    test.addAll((Vector<Example>)foldBins[i][foldnum]);
